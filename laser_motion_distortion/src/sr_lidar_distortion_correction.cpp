...
void SRLidarDistortionCorrection::pcdCallback(const sensor_msgs::PointCloud2ConstPtr &pc)
{
  ...
  if (imu_buffer_[0].timestamp < scan_start)
  {
    ...
    w_prev = (1.0 - alpha) * Eigen::Vector3d(imu_buffer_[0].wx, imu_buffer_[0].wy, imu_buffer_[0].wz) + alpha * Eigen::Vector3d(imu_buffer_[1].wx, imu_buffer_[1].wy, imu_buffer_[1].wz);
  }
  else
  {
  ...
  {
    if (imu_buffer_[i].timestamp <= scan_start)
      continue;
  ...
  for (unsigned int i = 0; i < pcd_in_->size(); ++i)
  {
    ...
    // check for validity
    if (std::isnan(scan_x) || std::isnan(scan_y) || std::isnan(scan_z))
    {
      continue;
    }
    // optional distance filter
    if (radius_filter != -1 && (scan_x * scan_x + scan_y * scan_y + scan_z * scan_z < radius_sqr_m))
    {
      continue;
    }
