...
std::pair<std::vector<Eigen::Vector3d>, std::vector<Eigen::Vector3d>> LIONode::construct_source_and_target_pc(const pcl::PointCloud<PointType>::Ptr &pc_in){
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, N),
            [&](const tbb::blocked_range<size_t> &range){
                ...
                distance.clear();
                ikdtree_ptr_->Nearest_Search(pc_in->points[i], kNN, nearest_neighbour, distance);
...
bool LIONode::registerPointsGICP(const pcl::PointCloud<PointType>::Ptr &pc_in, const Sophus::SE3d &initial_guess, Sophus::SE3d &registered_pose, double max_correspondence_distance, int max_num_iterations, double sigma){
...
auto [source, target] = construct_source_and_target_pc(pc_in);
...
static inline Eigen::Vector3d ComputeEigenvector1(const Eigen::Matrix3d &A,
    ...
    if (absM00 >= absM11) {
        if (std::max(absM00, absM01) > 0) {
            if (absM00 >= absM01) { m01 /= m00; m00 = 1.0 / std::sqrt(1 + m01*m01); m01 *= m00; }
            else { m00 /= m01; m01 = 1.0 / std::sqrt(1 + m00*m00); m00 *= m01; }
            return m01 * U - m00 * V;
        } else return U;
    } else {
        if (std::max(absM11, absM01) > 0) {
            if (absM11 >= absM01) { m01 /= m11; m11 = 1.0 / std::sqrt(1 + m01*m01); m01 *= m11; }
            else { m11 /= m01; m01 = 1.0 / std::sqrt(1 + m11*m11); m11 *= m01; }
            return m11 * U - m01 * V;
        } else return U;
    }
}
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, points.size()),
        [&](const tbb::blocked_range<size_t> &range){
            for (size_t i = range.begin(); i < range.end(); ++i){
                ...
                double len = n.norm();
                if (len > 1e-9) n /= len; else n = Eigen::Vector3d::UnitZ();
