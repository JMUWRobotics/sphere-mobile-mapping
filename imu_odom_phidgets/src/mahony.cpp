...
quaternion MahonyFilter::filter(const Vec3 &gyro, const Vec3 &acc, float dt)
{
    ...
    if (a_norm > 0.0)
    {
        ...
        // Must match Python implementation exactly
        Vec3 a_exp;
        a_exp.x = 2.0 * (q1 * q3 - q0 * q2); // TODO: potentially change to subscribed normal (e.g. passed by imuJasper.cpp instead of the quaternion dependent gravity direction (but in body frame))
