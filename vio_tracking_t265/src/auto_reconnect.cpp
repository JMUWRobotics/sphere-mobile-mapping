...
class passive_time_sync
{
    ...
    // The domain offset between the two clocks
    double A_offset;
...
{
  double timestamp = frame.get_timestamp() - passive_syncer.get_clock_offset();
...
int main(int argc, char **argv)
{
  ...
  */
  auto rs2FramesCallback = [&](const rs2::frame &frame)
  {
    ...
    // Estimate clock offset to the ROS Master
    passive_syncer.online_sync(frame.get_timestamp(), ros::Time::now().toSec() * 1000.0);
